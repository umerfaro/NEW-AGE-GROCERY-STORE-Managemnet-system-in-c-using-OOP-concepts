...
{  //update password
	...
	if (strcmp(n.c_str(), getPassword()) == 0) {
		Manager d;
		fstream obj("LahoreManager.dat", ios::in | ios::out | ios::binary);
...
void Manager2::managernamechnage() {
	//update username
	Manager temp, manager;
	bool usernamecheck = true;
	fstream obj("LahoreManager.dat", ios::binary | ios::in | ios::out);
	string n;

	cout << "Please enter updated username: ";
	cin >> n;
	ifstream fin("LahoreManager.dat", ios::binary);
	if (!fin) {
		usernamecheck = true;
	}
	else
...
bool Manager2::login2()
{
	Manager2 temp, manager;       //login ovverride use because polymorphism is being used
	string input;
	cout << "Enter username: ";
	cin >> input;
	...
	bool check = true;
	ifstream fileinput("LahoreManager.dat", ios::binary);
	if (!fileinput) {
...
{  //menu override used to because polymorphism is used
	...
	lable5:
		...
		case 5:
		{
			inventoryss.Write_THE_PRoduct(2);
			system("pause");
			goto lable5;
			break;
		}
		...
		case 7:
		{
			inventoryss.refillquantityofproduct(2);
			system("pause");
			goto lable5;
			break;
		}
		case 8:
		{
			inventoryss.modify_given_product_in_inventory(2);
			system("pause");
			goto lable5;
			break;
		}
		...
		case 10:
		{
			...
			case 1:
			{
				...
				cin >> s;
				inventoryss.search_seperate_product(s, 1);
				system("pause");
				goto lable5;
				break;
			...
			case 2:
			{
				...
				cout << "Enter product name to search" << endl;
				cin >> s;
				inventoryss.search_seperate_product(s, 3);
				system("pause");
				goto lable5;
				break;
...
void Manager2::Register() 
{
	...
	ifstream fileinput2("LahoreManager.dat", ios::binary);
	if (!fileinput2) {
		usernamecheck = true;
	}
	else
	...
	ofstream myfile("LahoreManager.dat", ios::binary | ios::app);
